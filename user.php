...
<div class="bg-cars">
    ...
    <?php
        ...
        if(isset($_SESSION['user_email'])) {
            ...
            echo "<div class='name'>Logged in as: {$row['name']} | $userEmail</div>";

            $sql2 = "SELECT * FROM vehicles WHERE owner='{$userEmail}'";
            $result = $conn->query($sql2);
            echo "<div class='table-vehicles-owned table-container'> <h1>Vehicles Owned <button id='parkNow' onclick=\"window.location.href='user.php?addVehicle=true'\">Add Vehicle</button></h1>";
            if ($result->num_rows == 0) {
                ...
                echo "<table border='1'>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr onclick=\"pop = document.getElementById('popup'); pop.style.display = 'block'; pop.querySelector('h2').innerText = '{$row['vehicle']}'; pop.querySelector('#carName').value='{$row['vehicle']}'\">
                            <td>{$row['vehicle']}</td>
                            <td>{$row['type']}</td>
                        </tr>";
                    }
            ...
            echo "<div class='table-vehicles-parked table-container'> <h1>Vehicles Parked<button id='parkNow' onclick=\"window.location.href='bookslot.php'\">Park Vehicle</button></h1></h1>";
            if ($result->num_rows == 0) {
            ...
            $result = $conn->query($sql3);
            echo "<div class='table-vehicles-parked table-container'> <h1>Your Bills<button id='parkNow' onclick=\"window.location.href='user.php?addReview=true'\">Add Review</button></h1> ";
            if ($result->num_rows == 0) {
